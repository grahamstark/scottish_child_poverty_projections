--
-- Created by ada_generator.py on 2017-09-14 11:23:39.955052
-- 
with Ukds;
with DB_Commons;
with Base_Types;
...
package Ukds.Frs.Accounts_IO is
   ...
   --
   -- returns true if the primary key parts of a_accounts match the defaults in Ukds.Frs.Null_Accounts
   --
   function Is_Null( a_accounts : Accounts ) return Boolean;
   
   --
   -- returns the single a_accounts matching the primary key fields, or the Ukds.Frs.Null_Accounts record
   -- if no such record exists
   --
   function Retrieve_By_PK( user_id : Integer; edition : Integer; year : Integer; sernum : Sernum_Value; benunit : Integer; person : Integer; account : Integer; connection : Database_Connection := null ) return Ukds.Frs.Accounts;
   --
   ...
   --
   -- Retrieves a list of Ukds.Frs.Accounts matching the criteria, or throws an exception
   --
   function Retrieve( c : d.Criteria; connection : Database_Connection := null ) return Ukds.Frs.Accounts_List;
   
   --
   -- Retrieves a list of Ukds.Frs.Accounts retrived by the sql string, or throws an exception
   --
   function Retrieve( sqlstr : String; connection : Database_Connection := null ) return Ukds.Frs.Accounts_List;
   
   --
   -- Save the given record, overwriting if it exists and overwrite is true, 
   -- otherwise throws DB_Exception exception. 
   --
   procedure Save( a_accounts : Ukds.Frs.Accounts; overwrite : Boolean := True; connection : Database_Connection := null );
   
   --
   -- Delete the given record. Throws DB_Exception exception. Sets value to Ukds.Frs.Null_Accounts
   --
   procedure Delete( a_accounts : in out Ukds.Frs.Accounts; connection : Database_Connection := null );
   --
   ...
   procedure Add_gtwtot( c : in out d.Criteria; gtwtot : Integer; op : d.operation_type:= d.eq; join : d.join_type := d.join_and );
   --
   ...
   procedure Add_gtwtot_To_Orderings( c : in out d.Criteria; direction : d.Asc_Or_Desc );

   function Map_From_Cursor( cursor : GNATCOLL.SQL.Exec.Forward_Cursor ) return Ukds.Frs.Accounts;
   ...
   --   11 : nsamt                    : Parameter_Integer  : Integer              :        0 
   --   12 : month                    : Parameter_Integer  : Integer              :        0 
   --   13 : issue                    : Parameter_Integer  : Integer              :        0 
   --   14 : gtwtot                   : Parameter_Integer  : Integer              :        0 
