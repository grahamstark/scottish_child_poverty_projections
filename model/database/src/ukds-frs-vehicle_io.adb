--
-- Created by ada_generator.py on 2017-09-21 20:55:38.321611
-- 
with Ukds;
...
package body Ukds.Frs.Vehicle_IO is
   ...
   log_trace : GNATColl.Traces.Trace_Handle := GNATColl.Traces.Create( "UKDS.FRS.VEHICLE_IO" );
   ...
   SELECT_PART : constant String := "select " &
         "user_id, edition, year, sernum, vehseq, vehic, vehown, month " &
         " from frs.vehicle " ;
   
   --
   -- Insert all variables; substring to be competed with output from some criteria
   --
   INSERT_PART : constant String := "insert into frs.vehicle (" &
         "user_id, edition, year, sernum, vehseq, vehic, vehown, month " &
         " ) values " ;
   ...
   --
   DELETE_PART : constant String := "delete from frs.vehicle ";
   
   --
   -- update
   --
   UPDATE_PART : constant String := "update frs.vehicle set  ";
   ...
   use GNATCOLL.SQL_Impl;
      params : constant SQL_Parameters( 1 .. 8 ) := ( if update_order then (
            1 => ( Parameter_Integer, 0 ),   --  : vehic (Integer)
            2 => ( Parameter_Integer, 0 ),   --  : vehown (Integer)
            ...
            7 => ( Parameter_Bigint, 0 ),   --  : sernum (Sernum_Value)
            8 => ( Parameter_Integer, 0 )   --  : vehseq (Integer)
      ) else (
            ...
            5 => ( Parameter_Integer, 0 ),   --  : vehseq (Integer)
            6 => ( Parameter_Integer, 0 ),   --  : vehic (Integer)
            7 => ( Parameter_Integer, 0 ),   --  : vehown (Integer)
   ...
   function Get_Prepared_Retrieve_Statement return gse.Prepared_Statement is 
      s : constant String := " where user_id = $1 and edition = $2 and year = $3 and sernum = $4 and vehseq = $5"; 
   begin 
   ...
   function Get_Prepared_Update_Statement return gse.Prepared_Statement is 
      ...
      query : constant String := DB_Commons.Add_Schema_To_Query( UPDATE_PART, SCHEMA_NAME ) & " vehic = $1, vehown = $2, month = $3 where user_id = $4 and edition = $5 and year = $6 and sernum = $7 and vehseq = $8"; 
   begin 
      ps := gse.Prepare( 
        query, 
        On_Server => True ); 
   ...
   Next_Free_user_id_query : constant String := 
         DB_Commons.Add_Schema_To_Query( "select coalesce( max( user_id ) + 1, 1 ) from frs.vehicle", SCHEMA_NAME );
   ...
   Next_Free_edition_query : constant String := 
         DB_Commons.Add_Schema_To_Query( "select coalesce( max( edition ) + 1, 1 ) from frs.vehicle", SCHEMA_NAME );
   ...
   Next_Free_year_query : constant String := 
         DB_Commons.Add_Schema_To_Query( "select coalesce( max( year ) + 1, 1 ) from frs.vehicle", SCHEMA_NAME );
   ...
   Next_Free_sernum_query : constant String := 
         DB_Commons.Add_Schema_To_Query( "select coalesce( max( sernum ) + 1, 1 ) from frs.vehicle", SCHEMA_NAME );
   ...
   Next_Free_vehseq_query : constant String := 
         DB_Commons.Add_Schema_To_Query( "select coalesce( max( vehseq ) + 1, 1 ) from frs.vehicle", SCHEMA_NAME );
   Next_Free_vehseq_ps : gse.Prepared_Statement := 
        gse.Prepare( Next_Free_vehseq_query, On_Server => True );
   -- 
   -- Next highest avaiable value of vehseq - useful for saving  
   --
   function Next_Free_vehseq( connection : Database_Connection := null) return Integer is
      cursor              : gse.Forward_Cursor;
      ai                  : Integer;
      local_connection    : Database_Connection;
      is_local_connection : Boolean;
   begin
      ...
      cursor.Fetch( local_connection, Next_Free_vehseq_ps );
      Check_Result( local_connection );
      if( gse.Has_Row( cursor ))then
         ai := gse.Integer_Value( cursor, 0, 0 );
      ...
      return ai;
   end Next_Free_vehseq;



   --
   -- returns true if the primary key parts of Ukds.Frs.Vehicle match the defaults in Ukds.Frs.Null_Vehicle
   --
   --
   -- Does this Ukds.Frs.Vehicle equal the default Ukds.Frs.Null_Vehicle ?
   --
   function Is_Null( a_vehicle : Vehicle ) return Boolean is
   begin
      return a_vehicle = Ukds.Frs.Null_Vehicle;
   end Is_Null;


   
   --
   -- returns the single Ukds.Frs.Vehicle matching the primary key fields, or the Ukds.Frs.Null_Vehicle record
   -- if no such record exists
   --
   function Retrieve_By_PK( user_id : Integer; edition : Integer; year : Integer; sernum : Sernum_Value; vehseq : Integer; connection : Database_Connection := null ) return Ukds.Frs.Vehicle is
      l : Ukds.Frs.Vehicle_List;
      a_vehicle : Ukds.Frs.Vehicle;
      c : d.Criteria;
   begin      
      ...
      if( not Ukds.Frs.Vehicle_List_Package.is_empty( l ) ) then
         a_vehicle := Ukds.Frs.Vehicle_List_Package.First_Element( l );
      else
         a_vehicle := Ukds.Frs.Null_Vehicle;
      end if;
      return a_vehicle;
   end Retrieve_By_PK;
   ...
   EXISTS_PS : constant gse.Prepared_Statement := gse.Prepare( 
        "select 1 from frs.vehicle where user_id = $1 and edition = $2 and year = $3 and sernum = $4 and vehseq = $5", 
        On_Server => True );
        
   function Exists( user_id : Integer; edition : Integer; year : Integer; sernum : Sernum_Value; vehseq : Integer; connection : Database_Connection := null ) return Boolean  is
   ...
   --
   -- Retrieves a list of Vehicle matching the criteria, or throws an exception
   --
   function Retrieve( c : d.Criteria; connection : Database_Connection := null ) return Ukds.Frs.Vehicle_List is
   begin      
   ...
   --
   -- Retrieves a list of Vehicle retrived by the sql string, or throws an exception
   --
   function Map_From_Cursor( cursor : gse.Forward_Cursor ) return Ukds.Frs.Vehicle is
      a_vehicle : Ukds.Frs.Vehicle;
   begin
      if not gse.Is_Null( cursor, 0 )then
         a_vehicle.user_id := gse.Integer_Value( cursor, 0 );
      end if;
      if not gse.Is_Null( cursor, 1 )then
         a_vehicle.edition := gse.Integer_Value( cursor, 1 );
      end if;
      if not gse.Is_Null( cursor, 2 )then
         a_vehicle.year := gse.Integer_Value( cursor, 2 );
      end if;
      if not gse.Is_Null( cursor, 3 )then
         a_vehicle.sernum := Sernum_Value'Value( gse.Value( cursor, 3 ));
      end if;
      if not gse.Is_Null( cursor, 4 )then
         a_vehicle.vehseq := gse.Integer_Value( cursor, 4 );
      end if;
      if not gse.Is_Null( cursor, 5 )then
         a_vehicle.vehic := gse.Integer_Value( cursor, 5 );
      end if;
      if not gse.Is_Null( cursor, 6 )then
         a_vehicle.vehown := gse.Integer_Value( cursor, 6 );
      end if;
      if not gse.Is_Null( cursor, 7 )then
         a_vehicle.month := gse.Integer_Value( cursor, 7 );
      end if;
      return a_vehicle;
   end Map_From_Cursor;

   function Retrieve( sqlstr : String; connection : Database_Connection := null ) return Ukds.Frs.Vehicle_List is
      l : Ukds.Frs.Vehicle_List;
      local_connection : Database_Connection;
      is_local_connection : Boolean;
   ...
   begin
      ...
      while gse.Has_Row( cursor ) loop
         declare
            a_vehicle : Ukds.Frs.Vehicle := Map_From_Cursor( cursor );
         begin
            l.append( a_vehicle ); 
         end;
         gse.Next( cursor );
   ...
   procedure Update( a_vehicle : Ukds.Frs.Vehicle; connection : Database_Connection := null ) is
   ...
   begin
      ...
      params( 1 ) := "+"( Integer'Pos( a_vehicle.vehic ));
      params( 2 ) := "+"( Integer'Pos( a_vehicle.vehown ));
      params( 3 ) := "+"( Integer'Pos( a_vehicle.month ));
      params( 4 ) := "+"( Integer'Pos( a_vehicle.user_id ));
      params( 5 ) := "+"( Integer'Pos( a_vehicle.edition ));
      params( 6 ) := "+"( Integer'Pos( a_vehicle.year ));
      params( 7 ) := As_Bigint( a_vehicle.sernum );
      params( 8 ) := "+"( Integer'Pos( a_vehicle.vehseq ));
   ...
   procedure Save( a_vehicle : Ukds.Frs.Vehicle; overwrite : Boolean := True; connection : Database_Connection := null ) is   
   ...
   begin
      ...
      if overwrite and Exists( a_vehicle.user_id, a_vehicle.edition, a_vehicle.year, a_vehicle.sernum, a_vehicle.vehseq ) then
         Update( a_vehicle, local_connection );
         if( is_local_connection )then
            Connection_Pool.Return_Connection( local_connection );
         end if;
         return;
      end if;
      params( 1 ) := "+"( Integer'Pos( a_vehicle.user_id ));
      params( 2 ) := "+"( Integer'Pos( a_vehicle.edition ));
      params( 3 ) := "+"( Integer'Pos( a_vehicle.year ));
      params( 4 ) := As_Bigint( a_vehicle.sernum );
      params( 5 ) := "+"( Integer'Pos( a_vehicle.vehseq ));
      params( 6 ) := "+"( Integer'Pos( a_vehicle.vehic ));
      params( 7 ) := "+"( Integer'Pos( a_vehicle.vehown ));
      params( 8 ) := "+"( Integer'Pos( a_vehicle.month ));
   ...
   procedure Delete( a_vehicle : in out Ukds.Frs.Vehicle; connection : Database_Connection := null ) is
         c : d.Criteria;
   begin  
      Add_user_id( c, a_vehicle.user_id );
      Add_edition( c, a_vehicle.edition );
      Add_year( c, a_vehicle.year );
      Add_sernum( c, a_vehicle.sernum );
      Add_vehseq( c, a_vehicle.vehseq );
      Delete( c, connection );
      a_vehicle := Ukds.Frs.Null_Vehicle;
   ...
   procedure Add_vehseq( c : in out d.Criteria; vehseq : Integer; op : d.operation_type:= d.eq; join : d.join_type := d.join_and ) is   
   elem : d.Criterion := d.Make_Criterion_Element( "vehseq", op, join, vehseq );
   begin
      d.add_to_criteria( c, elem );
   end Add_vehseq;


   procedure Add_vehic( c : in out d.Criteria; vehic : Integer; op : d.operation_type:= d.eq; join : d.join_type := d.join_and ) is   
   elem : d.Criterion := d.Make_Criterion_Element( "vehic", op, join, vehic );
   begin
      d.add_to_criteria( c, elem );
   end Add_vehic;


   procedure Add_vehown( c : in out d.Criteria; vehown : Integer; op : d.operation_type:= d.eq; join : d.join_type := d.join_and ) is   
   elem : d.Criterion := d.Make_Criterion_Element( "vehown", op, join, vehown );
   begin
      d.add_to_criteria( c, elem );
   end Add_vehown;
   ...
   procedure Add_vehseq_To_Orderings( c : in out d.Criteria; direction : d.Asc_Or_Desc ) is   
   elem : d.Order_By_Element := d.Make_Order_By_Element( "vehseq", direction  );
   begin
      d.add_to_criteria( c, elem );
   end Add_vehseq_To_Orderings;


   procedure Add_vehic_To_Orderings( c : in out d.Criteria; direction : d.Asc_Or_Desc ) is   
   elem : d.Order_By_Element := d.Make_Order_By_Element( "vehic", direction  );
   begin
      d.add_to_criteria( c, elem );
   end Add_vehic_To_Orderings;


   procedure Add_vehown_To_Orderings( c : in out d.Criteria; direction : d.Asc_Or_Desc ) is   
   elem : d.Order_By_Element := d.Make_Order_By_Element( "vehown", direction  );
   begin
      d.add_to_criteria( c, elem );
   end Add_vehown_To_Orderings;
