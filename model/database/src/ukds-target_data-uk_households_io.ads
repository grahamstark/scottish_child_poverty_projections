...
package Ukds.Target_Data.Uk_Households_IO is
   ...
   --
   -- returns true if the primary key parts of a_uk_households match the defaults in Ukds.Target_Data.Null_Uk_Households
   --
   function Is_Null( a_uk_households : Uk_Households ) return Boolean;
   
   --
   -- returns the single a_uk_households matching the primary key fields, or the Ukds.Target_Data.Null_Uk_Households record
   -- if no such record exists
   --
   function Retrieve_By_PK( year : Year_Number; rec_type : Unbounded_String; variant : Unbounded_String; country : Unbounded_String; edition : Year_Number; connection : Database_Connection := null ) return Ukds.Target_Data.Uk_Households;
   --
   ...
   --
   -- Retrieves a list of Ukds.Target_Data.Uk_Households matching the criteria, or throws an exception
   --
   function Retrieve( c : d.Criteria; connection : Database_Connection := null ) return Ukds.Target_Data.Uk_Households_List;
   
   --
   -- Retrieves a list of Ukds.Target_Data.Uk_Households retrived by the sql string, or throws an exception
   --
   function Retrieve( sqlstr : String; connection : Database_Connection := null ) return Ukds.Target_Data.Uk_Households_List;
   
   --
   -- Save the given record, overwriting if it exists and overwrite is true, 
   -- otherwise throws DB_Exception exception. 
   --
   procedure Save( a_uk_households : Ukds.Target_Data.Uk_Households; overwrite : Boolean := True; connection : Database_Connection := null );
   
   --
   -- Delete the given record. Throws DB_Exception exception. Sets value to Ukds.Target_Data.Null_Uk_Households
   --
   procedure Delete( a_uk_households : in out Ukds.Target_Data.Uk_Households; connection : Database_Connection := null );
   --
   ...
   function Map_From_Cursor( cursor : GNATCOLL.SQL.Exec.Forward_Cursor ) return Ukds.Target_Data.Uk_Households;
   ...
   --    6 : one_adult_hh             : Parameter_Float    : Amount               :      0.0 
   --    7 : two_adult_hh             : Parameter_Float    : Amount               :      0.0 
   --    8 : other_hh                 : Parameter_Float    : Amount               :      0.0 
