--
-- Created by ada_generator.py on 2017-09-06 17:20:42.316810
-- 
with Ukds;
...
package body Ukds.Frs.Pianon1011_IO is
   ...
   log_trace : GNATColl.Traces.Trace_Handle := GNATColl.Traces.Create( "UKDS.FRS.PIANON1011_IO" );
   ...
   --
   INSERT_PART : constant String := "insert into frs.pianon1011 (" &
         "user_id, edition, year, gvtregn, fambu, newfambu, sexhd, adultb, benunit, ethgrphh," &
         "gs_newbu, gs_newpp, mbhcdec, obhcdec, mahcdec, oahcdec, sexsp, newfamb2, pidefbhc, pidefahc," &
         "pigrosbu, pinincbu, pigoccbu, pippenbu, piginvbu, pigernbu, pibenibu, piothibu, pinahcbu, piirbbu," &
         "pidisben, piretben, pripenbu, nonben2bu, perbenbu, perbenbu2, rrpen, newfambu2, dummy, coup_q1," &
   ...
   --
   DELETE_PART : constant String := "delete from frs.pianon1011 ";
   
   --
   -- update
   --
   UPDATE_PART : constant String := "update frs.pianon1011 set  ";
   ...
   Next_Free_user_id_query : constant String := 
         DB_Commons.Add_Schema_To_Query( "select coalesce( max( user_id ) + 1, 1 ) from frs.pianon1011", SCHEMA_NAME );
   ...
   Next_Free_edition_query : constant String := 
         DB_Commons.Add_Schema_To_Query( "select coalesce( max( edition ) + 1, 1 ) from frs.pianon1011", SCHEMA_NAME );
   ...
   Next_Free_year_query : constant String := 
         DB_Commons.Add_Schema_To_Query( "select coalesce( max( year ) + 1, 1 ) from frs.pianon1011", SCHEMA_NAME );
   ...
   Next_Free_benunit_query : constant String := 
         DB_Commons.Add_Schema_To_Query( "select coalesce( max( benunit ) + 1, 1 ) from frs.pianon1011", SCHEMA_NAME );
   ...
   Next_Free_sernum_query : constant String := 
         DB_Commons.Add_Schema_To_Query( "select coalesce( max( sernum ) + 1, 1 ) from frs.pianon1011", SCHEMA_NAME );
   ...
   --
   -- returns true if the primary key parts of Ukds.Frs.Pianon1011 match the defaults in Ukds.Frs.Null_Pianon1011
   --
   --
   -- Does this Ukds.Frs.Pianon1011 equal the default Ukds.Frs.Null_Pianon1011 ?
   --
   function Is_Null( a_pianon1011 : Pianon1011 ) return Boolean is
   begin
      return a_pianon1011 = Ukds.Frs.Null_Pianon1011;
   end Is_Null;


   
   --
   -- returns the single Ukds.Frs.Pianon1011 matching the primary key fields, or the Ukds.Frs.Null_Pianon1011 record
   -- if no such record exists
   --
   function Retrieve_By_PK( user_id : Integer; edition : Integer; year : Integer; benunit : Integer; sernum : Sernum_Value; connection : Database_Connection := null ) return Ukds.Frs.Pianon1011 is
      l : Ukds.Frs.Pianon1011_List;
      a_pianon1011 : Ukds.Frs.Pianon1011;
      c : d.Criteria;
   begin      
      ...
      if( not Ukds.Frs.Pianon1011_List_Package.is_empty( l ) ) then
         a_pianon1011 := Ukds.Frs.Pianon1011_List_Package.First_Element( l );
      else
         a_pianon1011 := Ukds.Frs.Null_Pianon1011;
      end if;
      return a_pianon1011;
   end Retrieve_By_PK;
   ...
   EXISTS_PS : constant gse.Prepared_Statement := gse.Prepare( 
        "select 1 from frs.pianon1011 where user_id = $1 and edition = $2 and year = $3 and benunit = $4 and sernum = $5", 
        On_Server => True );
   ...
   --
   -- Retrieves a list of Pianon1011 matching the criteria, or throws an exception
   --
   function Retrieve( c : d.Criteria; connection : Database_Connection := null ) return Ukds.Frs.Pianon1011_List is
   begin      
   ...
   --
   -- Retrieves a list of Pianon1011 retrived by the sql string, or throws an exception
   --
   function Map_From_Cursor( cursor : gse.Forward_Cursor ) return Ukds.Frs.Pianon1011 is
      a_pianon1011 : Ukds.Frs.Pianon1011;
   begin
      if not gse.Is_Null( cursor, 0 )then
         a_pianon1011.user_id := gse.Integer_Value( cursor, 0 );
      end if;
      if not gse.Is_Null( cursor, 1 )then
         a_pianon1011.edition := gse.Integer_Value( cursor, 1 );
      end if;
      if not gse.Is_Null( cursor, 2 )then
         a_pianon1011.year := gse.Integer_Value( cursor, 2 );
      end if;
      if not gse.Is_Null( cursor, 3 )then
         a_pianon1011.gvtregn := gse.Integer_Value( cursor, 3 );
      end if;
      if not gse.Is_Null( cursor, 4 )then
         a_pianon1011.fambu := gse.Integer_Value( cursor, 4 );
      end if;
      if not gse.Is_Null( cursor, 5 )then
         a_pianon1011.newfambu := gse.Integer_Value( cursor, 5 );
      end if;
      if not gse.Is_Null( cursor, 6 )then
         a_pianon1011.sexhd := gse.Integer_Value( cursor, 6 );
      end if;
      if not gse.Is_Null( cursor, 7 )then
         a_pianon1011.adultb := gse.Integer_Value( cursor, 7 );
      end if;
      if not gse.Is_Null( cursor, 8 )then
         a_pianon1011.benunit := gse.Integer_Value( cursor, 8 );
      end if;
      if not gse.Is_Null( cursor, 9 )then
         a_pianon1011.ethgrphh := gse.Integer_Value( cursor, 9 );
      end if;
      if not gse.Is_Null( cursor, 10 )then
         a_pianon1011.gs_newbu := gse.Integer_Value( cursor, 10 );
      end if;
      if not gse.Is_Null( cursor, 11 )then
         a_pianon1011.gs_newpp := gse.Integer_Value( cursor, 11 );
      end if;
      if not gse.Is_Null( cursor, 12 )then
         a_pianon1011.mbhcdec := gse.Integer_Value( cursor, 12 );
      end if;
      if not gse.Is_Null( cursor, 13 )then
         a_pianon1011.obhcdec := gse.Integer_Value( cursor, 13 );
      end if;
      if not gse.Is_Null( cursor, 14 )then
         a_pianon1011.mahcdec := gse.Integer_Value( cursor, 14 );
      end if;
      if not gse.Is_Null( cursor, 15 )then
         a_pianon1011.oahcdec := gse.Integer_Value( cursor, 15 );
      end if;
      if not gse.Is_Null( cursor, 16 )then
         a_pianon1011.sexsp := gse.Integer_Value( cursor, 16 );
      end if;
      if not gse.Is_Null( cursor, 17 )then
         a_pianon1011.newfamb2 := gse.Integer_Value( cursor, 17 );
      end if;
      if not gse.Is_Null( cursor, 18 )then
         a_pianon1011.pidefbhc:= Amount'Value( gse.Value( cursor, 18 ));
      end if;
      if not gse.Is_Null( cursor, 19 )then
         a_pianon1011.pidefahc:= Amount'Value( gse.Value( cursor, 19 ));
      end if;
      if not gse.Is_Null( cursor, 20 )then
         a_pianon1011.pigrosbu := gse.Integer_Value( cursor, 20 );
      end if;
      if not gse.Is_Null( cursor, 21 )then
         a_pianon1011.pinincbu := gse.Integer_Value( cursor, 21 );
      end if;
      if not gse.Is_Null( cursor, 22 )then
         a_pianon1011.pigoccbu := gse.Integer_Value( cursor, 22 );
      end if;
      if not gse.Is_Null( cursor, 23 )then
         a_pianon1011.pippenbu := gse.Integer_Value( cursor, 23 );
      end if;
      if not gse.Is_Null( cursor, 24 )then
         a_pianon1011.piginvbu := gse.Integer_Value( cursor, 24 );
      end if;
      if not gse.Is_Null( cursor, 25 )then
         a_pianon1011.pigernbu := gse.Integer_Value( cursor, 25 );
      end if;
      if not gse.Is_Null( cursor, 26 )then
         a_pianon1011.pibenibu := gse.Integer_Value( cursor, 26 );
      end if;
      if not gse.Is_Null( cursor, 27 )then
         a_pianon1011.piothibu := gse.Integer_Value( cursor, 27 );
      end if;
      if not gse.Is_Null( cursor, 28 )then
         a_pianon1011.pinahcbu := gse.Integer_Value( cursor, 28 );
      end if;
      if not gse.Is_Null( cursor, 29 )then
         a_pianon1011.piirbbu := gse.Integer_Value( cursor, 29 );
      end if;
      if not gse.Is_Null( cursor, 30 )then
         a_pianon1011.pidisben := gse.Integer_Value( cursor, 30 );
      end if;
      if not gse.Is_Null( cursor, 31 )then
         a_pianon1011.piretben := gse.Integer_Value( cursor, 31 );
      end if;
      if not gse.Is_Null( cursor, 32 )then
         a_pianon1011.pripenbu := gse.Integer_Value( cursor, 32 );
      end if;
      if not gse.Is_Null( cursor, 33 )then
         a_pianon1011.nonben2bu := gse.Integer_Value( cursor, 33 );
      end if;
      if not gse.Is_Null( cursor, 34 )then
         a_pianon1011.perbenbu := gse.Integer_Value( cursor, 34 );
      end if;
      if not gse.Is_Null( cursor, 35 )then
         a_pianon1011.perbenbu2 := gse.Integer_Value( cursor, 35 );
      end if;
      if not gse.Is_Null( cursor, 36 )then
         a_pianon1011.rrpen := gse.Integer_Value( cursor, 36 );
      end if;
      if not gse.Is_Null( cursor, 37 )then
         a_pianon1011.newfambu2 := gse.Integer_Value( cursor, 37 );
      end if;
      if not gse.Is_Null( cursor, 38 )then
         a_pianon1011.dummy := gse.Integer_Value( cursor, 38 );
      end if;
      if not gse.Is_Null( cursor, 39 )then
         a_pianon1011.coup_q1:= Amount'Value( gse.Value( cursor, 39 ));
      end if;
      if not gse.Is_Null( cursor, 40 )then
         a_pianon1011.coup_q2:= Amount'Value( gse.Value( cursor, 40 ));
      end if;
      if not gse.Is_Null( cursor, 41 )then
         a_pianon1011.coup_q3:= Amount'Value( gse.Value( cursor, 41 ));
      end if;
      if not gse.Is_Null( cursor, 42 )then
         a_pianon1011.coup_q4:= Amount'Value( gse.Value( cursor, 42 ));
      end if;
      if not gse.Is_Null( cursor, 43 )then
         a_pianon1011.coup_q5:= Amount'Value( gse.Value( cursor, 43 ));
      end if;
      if not gse.Is_Null( cursor, 44 )then
         a_pianon1011.acou_q1:= Amount'Value( gse.Value( cursor, 44 ));
      end if;
      if not gse.Is_Null( cursor, 45 )then
         a_pianon1011.acou_q2:= Amount'Value( gse.Value( cursor, 45 ));
      end if;
      if not gse.Is_Null( cursor, 46 )then
         a_pianon1011.acou_q3:= Amount'Value( gse.Value( cursor, 46 ));
      end if;
      if not gse.Is_Null( cursor, 47 )then
         a_pianon1011.acou_q4:= Amount'Value( gse.Value( cursor, 47 ));
      end if;
      if not gse.Is_Null( cursor, 48 )then
         a_pianon1011.acou_q5:= Amount'Value( gse.Value( cursor, 48 ));
      end if;
      if not gse.Is_Null( cursor, 49 )then
         a_pianon1011.sing_q1:= Amount'Value( gse.Value( cursor, 49 ));
      end if;
      if not gse.Is_Null( cursor, 50 )then
         a_pianon1011.sing_q2:= Amount'Value( gse.Value( cursor, 50 ));
      end if;
      if not gse.Is_Null( cursor, 51 )then
         a_pianon1011.sing_q3:= Amount'Value( gse.Value( cursor, 51 ));
      end if;
      if not gse.Is_Null( cursor, 52 )then
         a_pianon1011.sing_q4:= Amount'Value( gse.Value( cursor, 52 ));
      end if;
      if not gse.Is_Null( cursor, 53 )then
         a_pianon1011.sing_q5:= Amount'Value( gse.Value( cursor, 53 ));
      end if;
      if not gse.Is_Null( cursor, 54 )then
         a_pianon1011.asin_q1:= Amount'Value( gse.Value( cursor, 54 ));
      end if;
      if not gse.Is_Null( cursor, 55 )then
         a_pianon1011.asin_q2:= Amount'Value( gse.Value( cursor, 55 ));
      end if;
      if not gse.Is_Null( cursor, 56 )then
         a_pianon1011.asin_q3:= Amount'Value( gse.Value( cursor, 56 ));
      end if;
      if not gse.Is_Null( cursor, 57 )then
         a_pianon1011.asin_q4:= Amount'Value( gse.Value( cursor, 57 ));
      end if;
      if not gse.Is_Null( cursor, 58 )then
         a_pianon1011.asin_q5:= Amount'Value( gse.Value( cursor, 58 ));
      end if;
      if not gse.Is_Null( cursor, 59 )then
         a_pianon1011.clust := gse.Integer_Value( cursor, 59 );
      end if;
      if not gse.Is_Null( cursor, 60 )then
         a_pianon1011.strat := gse.Integer_Value( cursor, 60 );
      end if;
      if not gse.Is_Null( cursor, 61 )then
         a_pianon1011.agehd80 := gse.Integer_Value( cursor, 61 );
      end if;
      if not gse.Is_Null( cursor, 62 )then
         a_pianon1011.agesp80 := gse.Integer_Value( cursor, 62 );
      end if;
      if not gse.Is_Null( cursor, 63 )then
         a_pianon1011.sernum := Sernum_Value'Value( gse.Value( cursor, 63 ));
      end if;
      return a_pianon1011;
   end Map_From_Cursor;

   function Retrieve( sqlstr : String; connection : Database_Connection := null ) return Ukds.Frs.Pianon1011_List is
      l : Ukds.Frs.Pianon1011_List;
      local_connection : Database_Connection;
      is_local_connection : Boolean;
   ...
   begin
      ...
      while gse.Has_Row( cursor ) loop
         declare
            a_pianon1011 : Ukds.Frs.Pianon1011 := Map_From_Cursor( cursor );
         begin
            l.append( a_pianon1011 ); 
         end;
         gse.Next( cursor );
      end loop;
      if( is_local_connection )then
   ...
   procedure Update( a_pianon1011 : Ukds.Frs.Pianon1011; connection : Database_Connection := null ) is
   ...
   begin
      ...
      params( 1 ) := "+"( Integer'Pos( a_pianon1011.gvtregn ));
      params( 2 ) := "+"( Integer'Pos( a_pianon1011.fambu ));
      params( 3 ) := "+"( Integer'Pos( a_pianon1011.newfambu ));
      params( 4 ) := "+"( Integer'Pos( a_pianon1011.sexhd ));
      params( 5 ) := "+"( Integer'Pos( a_pianon1011.adultb ));
      params( 6 ) := "+"( Integer'Pos( a_pianon1011.ethgrphh ));
      params( 7 ) := "+"( Integer'Pos( a_pianon1011.gs_newbu ));
      params( 8 ) := "+"( Integer'Pos( a_pianon1011.gs_newpp ));
      params( 9 ) := "+"( Integer'Pos( a_pianon1011.mbhcdec ));
      params( 10 ) := "+"( Integer'Pos( a_pianon1011.obhcdec ));
      params( 11 ) := "+"( Integer'Pos( a_pianon1011.mahcdec ));
      params( 12 ) := "+"( Integer'Pos( a_pianon1011.oahcdec ));
      params( 13 ) := "+"( Integer'Pos( a_pianon1011.sexsp ));
      params( 14 ) := "+"( Integer'Pos( a_pianon1011.newfamb2 ));
      params( 15 ) := "+"( Float( a_pianon1011.pidefbhc ));
      params( 16 ) := "+"( Float( a_pianon1011.pidefahc ));
      params( 17 ) := "+"( Integer'Pos( a_pianon1011.pigrosbu ));
      params( 18 ) := "+"( Integer'Pos( a_pianon1011.pinincbu ));
      params( 19 ) := "+"( Integer'Pos( a_pianon1011.pigoccbu ));
      params( 20 ) := "+"( Integer'Pos( a_pianon1011.pippenbu ));
      params( 21 ) := "+"( Integer'Pos( a_pianon1011.piginvbu ));
      params( 22 ) := "+"( Integer'Pos( a_pianon1011.pigernbu ));
      params( 23 ) := "+"( Integer'Pos( a_pianon1011.pibenibu ));
      params( 24 ) := "+"( Integer'Pos( a_pianon1011.piothibu ));
      params( 25 ) := "+"( Integer'Pos( a_pianon1011.pinahcbu ));
      params( 26 ) := "+"( Integer'Pos( a_pianon1011.piirbbu ));
      params( 27 ) := "+"( Integer'Pos( a_pianon1011.pidisben ));
      params( 28 ) := "+"( Integer'Pos( a_pianon1011.piretben ));
      params( 29 ) := "+"( Integer'Pos( a_pianon1011.pripenbu ));
      params( 30 ) := "+"( Integer'Pos( a_pianon1011.nonben2bu ));
      params( 31 ) := "+"( Integer'Pos( a_pianon1011.perbenbu ));
      params( 32 ) := "+"( Integer'Pos( a_pianon1011.perbenbu2 ));
      params( 33 ) := "+"( Integer'Pos( a_pianon1011.rrpen ));
      params( 34 ) := "+"( Integer'Pos( a_pianon1011.newfambu2 ));
      params( 35 ) := "+"( Integer'Pos( a_pianon1011.dummy ));
      params( 36 ) := "+"( Float( a_pianon1011.coup_q1 ));
      params( 37 ) := "+"( Float( a_pianon1011.coup_q2 ));
      params( 38 ) := "+"( Float( a_pianon1011.coup_q3 ));
      params( 39 ) := "+"( Float( a_pianon1011.coup_q4 ));
      params( 40 ) := "+"( Float( a_pianon1011.coup_q5 ));
      params( 41 ) := "+"( Float( a_pianon1011.acou_q1 ));
      params( 42 ) := "+"( Float( a_pianon1011.acou_q2 ));
      params( 43 ) := "+"( Float( a_pianon1011.acou_q3 ));
      params( 44 ) := "+"( Float( a_pianon1011.acou_q4 ));
      params( 45 ) := "+"( Float( a_pianon1011.acou_q5 ));
      params( 46 ) := "+"( Float( a_pianon1011.sing_q1 ));
      params( 47 ) := "+"( Float( a_pianon1011.sing_q2 ));
      params( 48 ) := "+"( Float( a_pianon1011.sing_q3 ));
      params( 49 ) := "+"( Float( a_pianon1011.sing_q4 ));
      params( 50 ) := "+"( Float( a_pianon1011.sing_q5 ));
      params( 51 ) := "+"( Float( a_pianon1011.asin_q1 ));
      params( 52 ) := "+"( Float( a_pianon1011.asin_q2 ));
      params( 53 ) := "+"( Float( a_pianon1011.asin_q3 ));
      params( 54 ) := "+"( Float( a_pianon1011.asin_q4 ));
      params( 55 ) := "+"( Float( a_pianon1011.asin_q5 ));
      params( 56 ) := "+"( Integer'Pos( a_pianon1011.clust ));
      params( 57 ) := "+"( Integer'Pos( a_pianon1011.strat ));
      params( 58 ) := "+"( Integer'Pos( a_pianon1011.agehd80 ));
      params( 59 ) := "+"( Integer'Pos( a_pianon1011.agesp80 ));
      params( 60 ) := "+"( Integer'Pos( a_pianon1011.user_id ));
      params( 61 ) := "+"( Integer'Pos( a_pianon1011.edition ));
      params( 62 ) := "+"( Integer'Pos( a_pianon1011.year ));
      params( 63 ) := "+"( Integer'Pos( a_pianon1011.benunit ));
      params( 64 ) := As_Bigint( a_pianon1011.sernum );
   ...
   procedure Save( a_pianon1011 : Ukds.Frs.Pianon1011; overwrite : Boolean := True; connection : Database_Connection := null ) is   
   ...
   begin
      ...
      if overwrite and Exists( a_pianon1011.user_id, a_pianon1011.edition, a_pianon1011.year, a_pianon1011.benunit, a_pianon1011.sernum ) then
         Update( a_pianon1011, local_connection );
         if( is_local_connection )then
            Connection_Pool.Return_Connection( local_connection );
         end if;
         return;
      end if;
      params( 1 ) := "+"( Integer'Pos( a_pianon1011.user_id ));
      params( 2 ) := "+"( Integer'Pos( a_pianon1011.edition ));
      params( 3 ) := "+"( Integer'Pos( a_pianon1011.year ));
      params( 4 ) := "+"( Integer'Pos( a_pianon1011.gvtregn ));
      params( 5 ) := "+"( Integer'Pos( a_pianon1011.fambu ));
      params( 6 ) := "+"( Integer'Pos( a_pianon1011.newfambu ));
      params( 7 ) := "+"( Integer'Pos( a_pianon1011.sexhd ));
      params( 8 ) := "+"( Integer'Pos( a_pianon1011.adultb ));
      params( 9 ) := "+"( Integer'Pos( a_pianon1011.benunit ));
      params( 10 ) := "+"( Integer'Pos( a_pianon1011.ethgrphh ));
      params( 11 ) := "+"( Integer'Pos( a_pianon1011.gs_newbu ));
      params( 12 ) := "+"( Integer'Pos( a_pianon1011.gs_newpp ));
      params( 13 ) := "+"( Integer'Pos( a_pianon1011.mbhcdec ));
      params( 14 ) := "+"( Integer'Pos( a_pianon1011.obhcdec ));
      params( 15 ) := "+"( Integer'Pos( a_pianon1011.mahcdec ));
      params( 16 ) := "+"( Integer'Pos( a_pianon1011.oahcdec ));
      params( 17 ) := "+"( Integer'Pos( a_pianon1011.sexsp ));
      params( 18 ) := "+"( Integer'Pos( a_pianon1011.newfamb2 ));
      params( 19 ) := "+"( Float( a_pianon1011.pidefbhc ));
      params( 20 ) := "+"( Float( a_pianon1011.pidefahc ));
      params( 21 ) := "+"( Integer'Pos( a_pianon1011.pigrosbu ));
      params( 22 ) := "+"( Integer'Pos( a_pianon1011.pinincbu ));
      params( 23 ) := "+"( Integer'Pos( a_pianon1011.pigoccbu ));
      params( 24 ) := "+"( Integer'Pos( a_pianon1011.pippenbu ));
      params( 25 ) := "+"( Integer'Pos( a_pianon1011.piginvbu ));
      params( 26 ) := "+"( Integer'Pos( a_pianon1011.pigernbu ));
      params( 27 ) := "+"( Integer'Pos( a_pianon1011.pibenibu ));
      params( 28 ) := "+"( Integer'Pos( a_pianon1011.piothibu ));
      params( 29 ) := "+"( Integer'Pos( a_pianon1011.pinahcbu ));
      params( 30 ) := "+"( Integer'Pos( a_pianon1011.piirbbu ));
      params( 31 ) := "+"( Integer'Pos( a_pianon1011.pidisben ));
      params( 32 ) := "+"( Integer'Pos( a_pianon1011.piretben ));
      params( 33 ) := "+"( Integer'Pos( a_pianon1011.pripenbu ));
      params( 34 ) := "+"( Integer'Pos( a_pianon1011.nonben2bu ));
      params( 35 ) := "+"( Integer'Pos( a_pianon1011.perbenbu ));
      params( 36 ) := "+"( Integer'Pos( a_pianon1011.perbenbu2 ));
      params( 37 ) := "+"( Integer'Pos( a_pianon1011.rrpen ));
      params( 38 ) := "+"( Integer'Pos( a_pianon1011.newfambu2 ));
      params( 39 ) := "+"( Integer'Pos( a_pianon1011.dummy ));
      params( 40 ) := "+"( Float( a_pianon1011.coup_q1 ));
      params( 41 ) := "+"( Float( a_pianon1011.coup_q2 ));
      params( 42 ) := "+"( Float( a_pianon1011.coup_q3 ));
      params( 43 ) := "+"( Float( a_pianon1011.coup_q4 ));
      params( 44 ) := "+"( Float( a_pianon1011.coup_q5 ));
      params( 45 ) := "+"( Float( a_pianon1011.acou_q1 ));
      params( 46 ) := "+"( Float( a_pianon1011.acou_q2 ));
      params( 47 ) := "+"( Float( a_pianon1011.acou_q3 ));
      params( 48 ) := "+"( Float( a_pianon1011.acou_q4 ));
      params( 49 ) := "+"( Float( a_pianon1011.acou_q5 ));
      params( 50 ) := "+"( Float( a_pianon1011.sing_q1 ));
      params( 51 ) := "+"( Float( a_pianon1011.sing_q2 ));
      params( 52 ) := "+"( Float( a_pianon1011.sing_q3 ));
      params( 53 ) := "+"( Float( a_pianon1011.sing_q4 ));
      params( 54 ) := "+"( Float( a_pianon1011.sing_q5 ));
      params( 55 ) := "+"( Float( a_pianon1011.asin_q1 ));
      params( 56 ) := "+"( Float( a_pianon1011.asin_q2 ));
      params( 57 ) := "+"( Float( a_pianon1011.asin_q3 ));
      params( 58 ) := "+"( Float( a_pianon1011.asin_q4 ));
      params( 59 ) := "+"( Float( a_pianon1011.asin_q5 ));
      params( 60 ) := "+"( Integer'Pos( a_pianon1011.clust ));
      params( 61 ) := "+"( Integer'Pos( a_pianon1011.strat ));
      params( 62 ) := "+"( Integer'Pos( a_pianon1011.agehd80 ));
      params( 63 ) := "+"( Integer'Pos( a_pianon1011.agesp80 ));
      params( 64 ) := As_Bigint( a_pianon1011.sernum );
   ...
   --
   -- Delete the given record. Throws DB_Exception exception. Sets value to Ukds.Frs.Null_Pianon1011
   --

   procedure Delete( a_pianon1011 : in out Ukds.Frs.Pianon1011; connection : Database_Connection := null ) is
         c : d.Criteria;
   begin  
      Add_user_id( c, a_pianon1011.user_id );
      Add_edition( c, a_pianon1011.edition );
      Add_year( c, a_pianon1011.year );
      Add_benunit( c, a_pianon1011.benunit );
      Add_sernum( c, a_pianon1011.sernum );
      Delete( c, connection );
      a_pianon1011 := Ukds.Frs.Null_Pianon1011;
