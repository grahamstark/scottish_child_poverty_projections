--
-- Created by ada_generator.py on 2017-09-20 15:38:49.043822
-- 
with Ukds;
...
package body Ukds.Frs.Pianon1314_IO is
   ...
   log_trace : GNATColl.Traces.Trace_Handle := GNATColl.Traces.Create( "UKDS.FRS.PIANON1314_IO" );
   ...
   --
   INSERT_PART : constant String := "insert into frs.pianon1314 (" &
         "user_id, edition, year, gvtregn, fambu, newfambu, sexhd, adultb, benunit, ethgrphh," &
         "gs_newbu, gs_newpp, mbhcdec, obhcdec, mahcdec, oahcdec, sexsp, newfamb2, pidefbhc, pidefahc," &
         "pigrosbu, pinincbu, pigoccbu, pippenbu, piginvbu, pigernbu, pibenibu, piothibu, pinahcbu, piirbbu," &
         "pidisben, piretben, pripenbu, nonben2bu, perbenbu, perbenbu2, rrpen, newfambu2, dummy, coup_q1," &
   ...
   --
   DELETE_PART : constant String := "delete from frs.pianon1314 ";
   
   --
   -- update
   --
   UPDATE_PART : constant String := "update frs.pianon1314 set  ";
   ...
   Next_Free_user_id_query : constant String := 
         DB_Commons.Add_Schema_To_Query( "select coalesce( max( user_id ) + 1, 1 ) from frs.pianon1314", SCHEMA_NAME );
   ...
   Next_Free_edition_query : constant String := 
         DB_Commons.Add_Schema_To_Query( "select coalesce( max( edition ) + 1, 1 ) from frs.pianon1314", SCHEMA_NAME );
   ...
   Next_Free_year_query : constant String := 
         DB_Commons.Add_Schema_To_Query( "select coalesce( max( year ) + 1, 1 ) from frs.pianon1314", SCHEMA_NAME );
   ...
   Next_Free_benunit_query : constant String := 
         DB_Commons.Add_Schema_To_Query( "select coalesce( max( benunit ) + 1, 1 ) from frs.pianon1314", SCHEMA_NAME );
   ...
   Next_Free_sernum_query : constant String := 
         DB_Commons.Add_Schema_To_Query( "select coalesce( max( sernum ) + 1, 1 ) from frs.pianon1314", SCHEMA_NAME );
   ...
   --
   -- returns true if the primary key parts of Ukds.Frs.Pianon1314 match the defaults in Ukds.Frs.Null_Pianon1314
   --
   --
   -- Does this Ukds.Frs.Pianon1314 equal the default Ukds.Frs.Null_Pianon1314 ?
   --
   function Is_Null( a_pianon1314 : Pianon1314 ) return Boolean is
   begin
      return a_pianon1314 = Ukds.Frs.Null_Pianon1314;
   end Is_Null;
   ...
   begin      
      ...
      if( not Ukds.Frs.Pianon1314_List_Package.is_empty( l ) ) then
         a_pianon1314 := Ukds.Frs.Pianon1314_List_Package.First_Element( l );
      else
         a_pianon1314 := Ukds.Frs.Null_Pianon1314;
      end if;
      return a_pianon1314;
   end Retrieve_By_PK;
   ...
   --
   -- Retrieves a list of Pianon1314 matching the criteria, or throws an exception
   --
   function Retrieve( c : d.Criteria; connection : Database_Connection := null ) return Ukds.Frs.Pianon1314_List is
   begin      
   ...
   --
   -- Retrieves a list of Pianon1314 retrived by the sql string, or throws an exception
   --
   function Map_From_Cursor( cursor : gse.Forward_Cursor ) return Ukds.Frs.Pianon1314 is
      a_pianon1314 : Ukds.Frs.Pianon1314;
   begin
      if not gse.Is_Null( cursor, 0 )then
         a_pianon1314.user_id := gse.Integer_Value( cursor, 0 );
      end if;
      if not gse.Is_Null( cursor, 1 )then
         a_pianon1314.edition := gse.Integer_Value( cursor, 1 );
      end if;
      if not gse.Is_Null( cursor, 2 )then
         a_pianon1314.year := gse.Integer_Value( cursor, 2 );
      end if;
      if not gse.Is_Null( cursor, 3 )then
         a_pianon1314.gvtregn := gse.Integer_Value( cursor, 3 );
      end if;
      if not gse.Is_Null( cursor, 4 )then
         a_pianon1314.fambu := gse.Integer_Value( cursor, 4 );
      end if;
      if not gse.Is_Null( cursor, 5 )then
         a_pianon1314.newfambu := gse.Integer_Value( cursor, 5 );
      end if;
      if not gse.Is_Null( cursor, 6 )then
         a_pianon1314.sexhd := gse.Integer_Value( cursor, 6 );
      end if;
      if not gse.Is_Null( cursor, 7 )then
         a_pianon1314.adultb := gse.Integer_Value( cursor, 7 );
      end if;
      if not gse.Is_Null( cursor, 8 )then
         a_pianon1314.benunit := gse.Integer_Value( cursor, 8 );
      end if;
      if not gse.Is_Null( cursor, 9 )then
         a_pianon1314.ethgrphh := gse.Integer_Value( cursor, 9 );
      end if;
      if not gse.Is_Null( cursor, 10 )then
         a_pianon1314.gs_newbu := gse.Integer_Value( cursor, 10 );
      end if;
      if not gse.Is_Null( cursor, 11 )then
         a_pianon1314.gs_newpp := gse.Integer_Value( cursor, 11 );
      end if;
      if not gse.Is_Null( cursor, 12 )then
         a_pianon1314.mbhcdec := gse.Integer_Value( cursor, 12 );
      end if;
      if not gse.Is_Null( cursor, 13 )then
         a_pianon1314.obhcdec := gse.Integer_Value( cursor, 13 );
      end if;
      if not gse.Is_Null( cursor, 14 )then
         a_pianon1314.mahcdec := gse.Integer_Value( cursor, 14 );
      end if;
      if not gse.Is_Null( cursor, 15 )then
         a_pianon1314.oahcdec := gse.Integer_Value( cursor, 15 );
      end if;
      if not gse.Is_Null( cursor, 16 )then
         a_pianon1314.sexsp := gse.Integer_Value( cursor, 16 );
      end if;
      ...
      if not gse.Is_Null( cursor, 21 )then
         a_pianon1314.pinincbu := gse.Integer_Value( cursor, 21 );
      end if;
      if not gse.Is_Null( cursor, 22 )then
         a_pianon1314.pigoccbu := gse.Integer_Value( cursor, 22 );
      end if;
      if not gse.Is_Null( cursor, 23 )then
         a_pianon1314.pippenbu := gse.Integer_Value( cursor, 23 );
      end if;
      if not gse.Is_Null( cursor, 24 )then
         a_pianon1314.piginvbu := gse.Integer_Value( cursor, 24 );
      end if;
      if not gse.Is_Null( cursor, 25 )then
         a_pianon1314.pigernbu := gse.Integer_Value( cursor, 25 );
      end if;
      if not gse.Is_Null( cursor, 26 )then
         a_pianon1314.pibenibu := gse.Integer_Value( cursor, 26 );
      end if;
      if not gse.Is_Null( cursor, 27 )then
         a_pianon1314.piothibu := gse.Integer_Value( cursor, 27 );
      end if;
      if not gse.Is_Null( cursor, 28 )then
         a_pianon1314.pinahcbu := gse.Integer_Value( cursor, 28 );
      end if;
      if not gse.Is_Null( cursor, 29 )then
         a_pianon1314.piirbbu := gse.Integer_Value( cursor, 29 );
      end if;
      if not gse.Is_Null( cursor, 30 )then
         a_pianon1314.pidisben := gse.Integer_Value( cursor, 30 );
      end if;
      if not gse.Is_Null( cursor, 31 )then
         a_pianon1314.piretben := gse.Integer_Value( cursor, 31 );
      end if;
      if not gse.Is_Null( cursor, 32 )then
         a_pianon1314.pripenbu := gse.Integer_Value( cursor, 32 );
      end if;
      if not gse.Is_Null( cursor, 33 )then
         a_pianon1314.nonben2bu := gse.Integer_Value( cursor, 33 );
      end if;
      if not gse.Is_Null( cursor, 34 )then
         a_pianon1314.perbenbu := gse.Integer_Value( cursor, 34 );
      end if;
      if not gse.Is_Null( cursor, 35 )then
         a_pianon1314.perbenbu2 := gse.Integer_Value( cursor, 35 );
      end if;
      if not gse.Is_Null( cursor, 36 )then
         a_pianon1314.rrpen := gse.Integer_Value( cursor, 36 );
      end if;
      if not gse.Is_Null( cursor, 37 )then
         a_pianon1314.newfambu2 := gse.Integer_Value( cursor, 37 );
      end if;
      ...
      if not gse.Is_Null( cursor, 60 )then
         a_pianon1314.strat := gse.Integer_Value( cursor, 60 );
      end if;
      if not gse.Is_Null( cursor, 61 )then
         a_pianon1314.agehd80 := gse.Integer_Value( cursor, 61 );
      end if;
   ...
   function Retrieve( sqlstr : String; connection : Database_Connection := null ) return Ukds.Frs.Pianon1314_List is
      l : Ukds.Frs.Pianon1314_List;
      local_connection : Database_Connection;
      is_local_connection : Boolean;
   ...
   begin
      ...
      while gse.Has_Row( cursor ) loop
         declare
            a_pianon1314 : Ukds.Frs.Pianon1314 := Map_From_Cursor( cursor );
         begin
            l.append( a_pianon1314 ); 
         end;
         gse.Next( cursor );
      end loop;
      if( is_local_connection )then
   ...
   procedure Update( a_pianon1314 : Ukds.Frs.Pianon1314; connection : Database_Connection := null ) is
   ...
   begin
      ...
      params( 1 ) := "+"( Integer'Pos( a_pianon1314.gvtregn ));
      params( 2 ) := "+"( Integer'Pos( a_pianon1314.fambu ));
      params( 3 ) := "+"( Integer'Pos( a_pianon1314.newfambu ));
      params( 4 ) := "+"( Integer'Pos( a_pianon1314.sexhd ));
      params( 5 ) := "+"( Integer'Pos( a_pianon1314.adultb ));
      params( 6 ) := "+"( Integer'Pos( a_pianon1314.ethgrphh ));
      params( 7 ) := "+"( Integer'Pos( a_pianon1314.gs_newbu ));
      params( 8 ) := "+"( Integer'Pos( a_pianon1314.gs_newpp ));
      params( 9 ) := "+"( Integer'Pos( a_pianon1314.mbhcdec ));
      params( 10 ) := "+"( Integer'Pos( a_pianon1314.obhcdec ));
      params( 11 ) := "+"( Integer'Pos( a_pianon1314.mahcdec ));
      ...
      params( 20 ) := "+"( Integer'Pos( a_pianon1314.pippenbu ));
      params( 21 ) := "+"( Integer'Pos( a_pianon1314.piginvbu ));
      params( 22 ) := "+"( Integer'Pos( a_pianon1314.pigernbu ));
      params( 23 ) := "+"( Integer'Pos( a_pianon1314.pibenibu ));
      params( 24 ) := "+"( Integer'Pos( a_pianon1314.piothibu ));
      params( 25 ) := "+"( Integer'Pos( a_pianon1314.pinahcbu ));
      params( 26 ) := "+"( Integer'Pos( a_pianon1314.piirbbu ));
      params( 27 ) := "+"( Integer'Pos( a_pianon1314.pidisben ));
      params( 28 ) := "+"( Integer'Pos( a_pianon1314.piretben ));
      params( 29 ) := "+"( Integer'Pos( a_pianon1314.pripenbu ));
      params( 30 ) := "+"( Integer'Pos( a_pianon1314.nonben2bu ));
      params( 31 ) := "+"( Integer'Pos( a_pianon1314.perbenbu ));
      params( 32 ) := "+"( Integer'Pos( a_pianon1314.perbenbu2 ));
      ...
      params( 59 ) := "+"( Integer'Pos( a_pianon1314.agesp80 ));
      params( 60 ) := "+"( Integer'Pos( a_pianon1314.user_id ));
   ...
   procedure Save( a_pianon1314 : Ukds.Frs.Pianon1314; overwrite : Boolean := True; connection : Database_Connection := null ) is   
   ...
   begin
      ...
      if overwrite and Exists( a_pianon1314.user_id, a_pianon1314.edition, a_pianon1314.year, a_pianon1314.benunit, a_pianon1314.sernum ) then
         Update( a_pianon1314, local_connection );
         if( is_local_connection )then
            Connection_Pool.Return_Connection( local_connection );
         end if;
         return;
      end if;
      params( 1 ) := "+"( Integer'Pos( a_pianon1314.user_id ));
      params( 2 ) := "+"( Integer'Pos( a_pianon1314.edition ));
      params( 3 ) := "+"( Integer'Pos( a_pianon1314.year ));
      params( 4 ) := "+"( Integer'Pos( a_pianon1314.gvtregn ));
      params( 5 ) := "+"( Integer'Pos( a_pianon1314.fambu ));
      params( 6 ) := "+"( Integer'Pos( a_pianon1314.newfambu ));
      params( 7 ) := "+"( Integer'Pos( a_pianon1314.sexhd ));
      params( 8 ) := "+"( Integer'Pos( a_pianon1314.adultb ));
      params( 9 ) := "+"( Integer'Pos( a_pianon1314.benunit ));
      params( 10 ) := "+"( Integer'Pos( a_pianon1314.ethgrphh ));
      params( 11 ) := "+"( Integer'Pos( a_pianon1314.gs_newbu ));
      params( 12 ) := "+"( Integer'Pos( a_pianon1314.gs_newpp ));
      params( 13 ) := "+"( Integer'Pos( a_pianon1314.mbhcdec ));
      params( 14 ) := "+"( Integer'Pos( a_pianon1314.obhcdec ));
      params( 15 ) := "+"( Integer'Pos( a_pianon1314.mahcdec ));
      ...
      params( 24 ) := "+"( Integer'Pos( a_pianon1314.pippenbu ));
      params( 25 ) := "+"( Integer'Pos( a_pianon1314.piginvbu ));
      params( 26 ) := "+"( Integer'Pos( a_pianon1314.pigernbu ));
      params( 27 ) := "+"( Integer'Pos( a_pianon1314.pibenibu ));
      params( 28 ) := "+"( Integer'Pos( a_pianon1314.piothibu ));
      params( 29 ) := "+"( Integer'Pos( a_pianon1314.pinahcbu ));
      params( 30 ) := "+"( Integer'Pos( a_pianon1314.piirbbu ));
      params( 31 ) := "+"( Integer'Pos( a_pianon1314.pidisben ));
      params( 32 ) := "+"( Integer'Pos( a_pianon1314.piretben ));
      params( 33 ) := "+"( Integer'Pos( a_pianon1314.pripenbu ));
      params( 34 ) := "+"( Integer'Pos( a_pianon1314.nonben2bu ));
      params( 35 ) := "+"( Integer'Pos( a_pianon1314.perbenbu ));
      params( 36 ) := "+"( Integer'Pos( a_pianon1314.perbenbu2 ));
   ...
   --
   -- Delete the given record. Throws DB_Exception exception. Sets value to Ukds.Frs.Null_Pianon1314
   --

   procedure Delete( a_pianon1314 : in out Ukds.Frs.Pianon1314; connection : Database_Connection := null ) is
         c : d.Criteria;
   begin  
      Add_user_id( c, a_pianon1314.user_id );
      Add_edition( c, a_pianon1314.edition );
      Add_year( c, a_pianon1314.year );
      Add_benunit( c, a_pianon1314.benunit );
      Add_sernum( c, a_pianon1314.sernum );
      Delete( c, connection );
      a_pianon1314 := Ukds.Frs.Null_Pianon1314;
